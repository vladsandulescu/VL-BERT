...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        max_question_length = max([len(data[self.data_names.index('text')]) for data in batch])
        ...
        for i, ibatch in enumerate(batch):
            ...
            question = ibatch[self.data_names.index('text')]
            out['text'] = clip_pad_1d(question, max_question_length, pad=0)
...
class BatchPairedCollator(object):
    def __init__(self, dataset, append_ind=False):
        self.dataset = dataset
        self.test_mode = self.dataset.test_mode
        self.data_names = self.dataset.data_names
    ...
    def __call__(self, batch):
        ...
        if batch[0][self.data_names.index('image')] is not None:
            ...
            image_none = True
        max_boxes = max([data[self.data_names.index('boxes')].shape[0] for data in batch])
        max_text_length = max([max([len(data[self.data_names.index('text')]),
                                   len(data[self.data_names.index('im2text')])]) for data in batch])
        ...
        for i, ibatch in enumerate(batch):
            ...
            question = ibatch[self.data_names.index('text')]
            out['text'] = clip_pad_1d(question, max_text_length, pad=0)

            im2text = ibatch[self.data_names.index('im2text')]
            out['im2text'] = clip_pad_1d(im2text, max_text_length, pad=0)
